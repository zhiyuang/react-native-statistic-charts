/**
 * https://github.com/antvis/F2/blob/%40antv/f2%404.0.29/packages/f2/src/controller/scale/linear-tick.ts
 */

import type { TickMethod } from '@antv/scale'
// 认为是nice的刻度
const SNAP_COUNT_ARRAY = [1, 1.2, 1.5, 2, 2.2, 2.4, 2.5, 3, 4, 5, 6, 7.5, 8, 10]
const DEFAULT_COUNT = 5 // 默认刻度值

const linearTick: TickMethod<number> = (min, max, count = DEFAULT_COUNT) => {
  min = isNaN(min) ? 0 : min
  max = isNaN(max) ? 0 : max
  ...
  const interval = getBestInterval({ tickCount: count, max, min })
  // 通过interval计算最小tick
...
function getFactor(number: number) {
  // 取正数
...
// 获取最佳匹配刻度
function getBestInterval({ tickCount, min, max }: { tickCount: number; min: number; max: number }) {
  // 如果最大最小相等，则直接按1处理
  if (min === max) {
...
function getInterval(startIndex: number, tickCount: number, min: number, max: number): number {
  let verify = false
...
// 计算小数点应该保留的位数
function getFixedLength(num: number) {
